import { SearchResult, ProductWithQuantity, SearchResultsEntity } from "../interfaces";
...
const ChooseItemsPage = ({ data, setPage }: IProps) => {
  ...
  const existingProducts = useSelector((state: any) => state.selectedProduct.products);

  const handleAddRemoveProduct = (result: SearchResultsEntity, quantity:number=10) => {
    setSelectedProducts((prevProducts) => {
      const existingProductIndex = prevProducts.findIndex((p) => p.product.product_code === result.product_code);
        ...
        <div key={productIndex} className="rounded-md bg-gray-200 p-4">
          ...
          <div className="max-w-[1200px] overflow-scroll">
            <div className="flex w-full">
              {product.search_results?.sort((a,b)=> b.score-a.score).slice(0,3).map((result, resultIndex) => {
                const selectedP = selectedProducts.some((p) => p.product.product_code === result.product_code);
                const selectedPVal = selectedProducts.find((p) => p.product.product_code === result.product_code);

                return (
                  <label
                    key={resultIndex}
                    onClick={() => { handleAddRemoveProduct(result,Number(product.quantity)); }}
                    className={`bg-white rounded-md flex flex-col justify-between m-2 p-2 min-h-80 cursor-pointer border-2 ${selectedP ? "border-blue-500" : "border-transparent"
                      }`}
                  >
                    <div>
                      ...
                      <p className="font-bold py-2">
                        {result.product_name && result.product_name.length > 20
                    ...
                    <div>
                      <p className="pb-2">
                        {result.product_code && result.product_code.length > 25
                      ...
                      {selectedPVal && (
                        <div className="flex items-center justify-between mt-4">
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleQuantityChange(result.product_code, "decrease");
                            }}
                          >
                            -
                          </button>
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleQuantityChange(result.product_code, "increase");
                            }}
                          >
                            +
                          </button>
